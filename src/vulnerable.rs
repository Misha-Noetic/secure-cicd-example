...
//
// dangerous_insecure_decode skips signature verification entirely.
// An attacker can forge tokens with arbitrary claims.
// ─────────────────────────────────────────────────────────────────────────────
#[get("/jwt-decode")]
pub async fn jwt_insecure(query: web::Query<TokenQuery>) -> impl Responder {
    // BAD: Skips signature verification — attacker can forge tokens
    let token_data = jsonwebtoken::dangerous_insecure_decode::<serde_json::Value>(
        &query.token,
    );
    match token_data {
